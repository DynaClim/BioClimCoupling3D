...
def Run_def_FT(FT,T,Forced_rc=True,rc=1):
    ...
    #mg          = 24*np.exp(0.64)*np.exp(-59640.505/(Topt*R))*Vc**0.67        # Maximum metabolic rate (d-1) (Gillooly 2001)
    mg          = 24*np.exp(-46.72+0.08*Topt)*Vc**(0.67)  # Maintenance (J.Cell-1.d-1) (Tijhuis 93 + Litchman 2007)
    #mg          = 24*np.exp(-46.72+0.08*298)*np.exp(0.2*(Topt-298))*Vc**(0.67)
    #qmax        = np.exp(-55.76+0.1*298+(0.85*59640.505-5920)/(8.41*298))*np.exp(-(0.85*59640.505-5920)/(R*Topt))*Vc**0.82
...
#    qmax   = traits[5]*(10*thresh-X0)/(8*thresh)
    ...
    if FT != 'photoH2':
        ...
        return(dNC,dX0,dX0D,qana,qcat,dgcat,dgana,ft.dgdiss,ft.Catabolism,ft.Anabolism,Lam)
