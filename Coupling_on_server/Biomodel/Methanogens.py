...
def Dgdiss(NoC,gamma):
    """
    Returns the dissipated energy during metabolism **REFERENCE**
    """
...
def Gamma(thresh,slope,gmax,x,Qc):
    ...
    """
    if x > 2*thresh:
        gamma = gmax/(1+np.exp(-slope*(np.log10((x-2*thresh)/thresh))))
        #gamma = gmax/(1+np.exp(-slope*np.log10(x/(2*thresh)/(10**(8*slope**-1)))))
    else:
        gamma = 0
    return(gamma)
